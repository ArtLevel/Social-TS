...
export const ProfileInfo: FC<IProfileInfo> = ({ isOwner }) => {
	...
	const socialLinks = profile?.contacts && Object.keys(profile.contacts)
		.filter(k => profile.contacts[k])
		.map(social => <a href={profile.contacts[social]} target='_blank'>{social}</a>)
					...
					<AboutMeWrapper>
						...
						<AboutMeBlock>
							...
							<AboutMeItem>
								...
								<span>{profile.lookingForAJob ? 'Yes, I am look for a new job !' : 'no'}</span>
							</AboutMeItem>
							<AboutMeItem>
